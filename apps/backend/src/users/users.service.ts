...
import {CreateUserDto, UpdateUserDto, User, UserRole} from '@org/shared-types';
...
export class UsersService {
  ...
  async findDeleted(query: PaginateQuery): Promise<Paginated<User>> {
    return paginate(query, this.userRepository, {
      sortableColumns: ['id', 'username', 'email', 'deletedAt'],
      nullSort: 'last',
  ...
  async findByRole(role: UserRole, query: PaginateQuery): Promise<Paginated<User>> {
    return paginate(query, this.userRepository, {
      sortableColumns: ['id', 'username', 'email', 'createdAt'],
      nullSort: 'last',
      defaultSortBy: [['createdAt', 'DESC']],
